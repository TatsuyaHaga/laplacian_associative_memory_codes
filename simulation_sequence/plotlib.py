...
def plot_color_network_target(filename, G, pos, color_list, targ, title=None):
    pylab.figure(figsize=(4,3))
    color_list=color_list[G.nodes()]
    color_max=numpy.max([numpy.max(color_list), numpy.abs(numpy.min(color_list))])
    nc=networkx.draw_networkx_nodes(G,pos,node_color=list(color_list),cmap="bwr", vmin=-color_max, vmax=color_max, node_size=50)
    networkx.draw_networkx_nodes(G,pos,nodelist=[int(targ)], node_color="black",node_shape="v", node_size=10)
    networkx.draw_networkx_edges(G,pos)
    if title!=None:
        pylab.title(title,fontsize=20)
    pylab.axis("off")
...
def plot_overlap_withparam_withtarget(filename, m_log, targ_log, param):
    pylab.figure(figsize=(4,4))
    pylab.subplot2grid((3,1),(0,0),rowspan=1)
    pylab.plot(param)
    pylab.xlim([0,len(param)-1])
    ...
    pylab.subplot2grid((3,1),(1,0),rowspan=2)
    pylab.imshow(m_log.T, interpolation="none", aspect="auto", cmap="jet")
    pylab.plot(targ_log, "--", color="white")
    pylab.xlabel("Time")
    pylab.ylabel("Pattern")
    #pylab.colorbar()
    pylab.tight_layout()
...
def plot_pattern_cor(filename, cor, title=None):
    ...
    pylab.xticks([1,P])
    pylab.yticks([1,P])
    pylab.xlabel("Trigger stimulus")
    pylab.ylabel("Trigger stimulus")
    pylab.colorbar()
    if title!=None:
        pylab.title(title, fontsize=20)
    pylab.tight_layout()

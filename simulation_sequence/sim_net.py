...
class sim_net():
    ...
    def simulate_single_changeparam(self,a_array,eta,simlen,start_node):
    ...
    def simulate_single_feedback(self,target_arr,eta,simlen,start_node, max_a, min_a):
        ...
        self.x=self.xi[:,start_node]+0.0
        self.m=(self.xi_bias.T@self.x)/(self.N*self.V)
        match=numpy.max([self.m[target_arr[0]],0])
        self.m_log=numpy.zeros([simlen,self.P])
        self.a_log=numpy.zeros(simlen)
        for t in range(simlen):
        ...
        return (self.m_log, self.a_log)

    def simulate_single_stim(self,a,eta,simlen,start_node,stim_pos,stim_amp):
        if len(stim_pos)!=simlen or len(stim_amp)!=simlen:
            print("Error: length of input array must be matched to simlen.")
            exit()
        #simulation
        self._set_weight(a)
        self.x=self.xi[:,start_node]+0.0
        ...
        for t in range(simlen):
            if t%100==0:
                print(t)
            stim=self.xi_bias[:,stim_pos[t]]*stim_amp[t]
            self.r=self._step(self.W@self.x+stim)
            self.x+=eta*(self.r-self.x)
            self.m=(self.xi_bias.T@self.x)/(self.N*self.V)
            self.m_log[t,:]=self.m
